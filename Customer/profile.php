...
/* ============================
   ...
   ============================ */
$stmt = $conn->prepare("
  SELECT full_name, email, contact_number, address, profile_image, role, reward_points, Resident_Address 
  FROM users 
...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
  ...
  if (!empty($_FILES['profile_image']['name'])) {
    ...
    $stmt = $conn->prepare("
      UPDATE users 
      SET full_name=?, email=?, contact_number=?, address=?, profile_image=?, Resident_Address=? 
      WHERE user_id=?
    ");
    $stmt->bind_param("sssssi", $full_name, $email, $contact, $resident_address, $address, $img_name, $user_id);
  } else {
    $stmt = $conn->prepare("
      UPDATE users 
      SET full_name=?, email=?, contact_number=?, resident_address=?, address=? 
      WHERE user_id=?
    ");
    $stmt->bind_param("ssssi", $full_name, $email, $contact, $resident_address, $address, $user_id);
  }
    ...
    <div class="card">
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="input-group">
          ...
          <input type="text" name="resident_address" value="<?= htmlspecialchars($user['Resident_Address']) ?>">
        </div>
